...
def connect(lst_box_archivos_SD, selectWindow):
    global send_file
    print("conectar")
    try:
        port = lst_box_archivos_SD.curselection()
        port = lst_box_archivos_SD.get(port)
        send_file = port
        selectWindow.destroy()
    except:
        pass

def update(lst_box_archivos_SD):
    pass
"""    lst_box_archivos_SD.delete(0,"end")
    puerto = serial_ports()
    for i in puerto :
        print("puerto disponibles : " + i)
        lst_box_archivos_SD.insert(1,i)  
    lst_box_archivos_SD.config(font = ("",content_size_font))
    lst_box_archivos_SD.pack()"""
...
def run_select_file(selectWindow, root):
    ...
    lst_box_archivos_SD = Listbox(selectWindow)
    #lst_box_archivos_SD.grid(row=0, column=0)
    sdFile = checkSDFiles()
    for i in sdFile :
        print("puerto disponibles : " + i)
        lst_box_archivos_SD.insert(1,i)  
    ...
    #interfaz grafica
    btn_connect = Button(selectWindow,text ="seleccionar", command = lambda : connect(lst_box_archivos_SD, selectWindow), )
    ...
    btn_update = Button(selectWindow, text ="Actualizar", command = lambda : update(lst_box_archivos_SD) )
